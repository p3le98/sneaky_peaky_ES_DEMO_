...
export class MetadataProtection {
  ...
  async protectMessage(options: {
    ...
    const embeddedContent = await this.embedMetadata(
      options.content,
      options.metadata || {},
      messageId
    );
  ...
  async unprotectMessage(options: {
    ...
    // Extract metadata
    const { originalContent, metadata } = await this.extractMetadata(
      unpaddedContent,
      options.messageId
    );
   ...
   * Apply padding to prevent traffic analysis
   */
  private applyPadding(content: string): {
    ...
    const targetLength = Math.min(
      Math.ceil(contentLength / this.PADDING_BLOCK_SIZE) * this.PADDING_BLOCK_SIZE,
      contentLength + this.MAX_PADDING_SIZE
    );
   ...
   */
  private removePadding(paddedContent: string): string {
   ...
   */
  private async applyTimingDelay(delayMs: number): Promise<void> {
   ...
   */
  private async deriveMetadataKey(messageId: string): Promise<CryptoKey> {
   ...
   */
  private byteToHex(bytes: Uint8Array, start: number, end: number): string {
