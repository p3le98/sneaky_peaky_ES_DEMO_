...
export class SecureStorage {
  ...
  private storageKeys: {
    normal: CryptoKey | null;
    high: CryptoKey | null;
    critical: CryptoKey | null;
  } = {
    normal: null,
    high: null,
    critical: null
  ...
  async store(options: {
    key: string;
    data: any;
    keySensitivity?: 'normal' | 'high' | 'critical';
  ...
  async retrieve(options: {
    ...
    try {
      ...
      const decryptedData = await this.decryptWithKey(
        encryptedDataBuffer,
        dek,
        ivBuffer
      );
   ...
   */
  private async deriveStorageKeys(): Promise<void> {
    if (!this.masterKey) {
      throw new Error('Master key not available');
    }
    
    // Derive keys for different sensitivity levels
    this.storageKeys.normal = await this.deriveKey(
      this.masterKey,
      'storage-key-normal'
    );
    
    this.storageKeys.high = await this.deriveKey(
      this.masterKey,
      'storage-key-high'
    );
    
    this.storageKeys.critical = await this.deriveKey(
      this.masterKey,
      'storage-key-critical'
    );
    ...
    const keyMaterial = await window.crypto.subtle.deriveBits(
      {
        name: 'HKDF',
        hash: 'SHA-256',
        salt: new Uint8Array(32), // A fixed salt; could be more sophisticated
...
/**
 * Interface for encrypted data stored in secure storage
 */
interface EncryptedData {
  ...
  metadata: {
    ...
    // When the data expires (null for no expiration)
    expires: number | null;
