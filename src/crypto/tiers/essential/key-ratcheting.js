...
class KeyRatcheting {
     ...
     */
    async createRatchetForRecipient(recipientId) {
     ...
     */
    async dhRatchetStep(state, remotePublicKey) {
        ...
        const sharedSecret = await this.calculateDHSecret(state.senderDHKeyPair.privateKey, remotePublicKey);
        ...
        const newSharedSecret = await this.calculateDHSecret(state.senderDHKeyPair.privateKey, remotePublicKey);
        // Derive new root key and sending chain key
        const { rootKey: newRootKey, chainKey: newChainKey } = await this.kdfRK(state.rootKey, newSharedSecret);
     ...
     */
    async getEncryptionKey(recipientId) {
     ...
     */
    async advanceRatchet(recipientId) {
    ...
    async generateRandomKey() {
        ...
        return window.crypto.subtle.importKey('raw', keyMaterial, { name: 'HKDF' }, false, ['deriveBits', 'deriveKey']);
    }
    /**
     ...
     */
    async calculateDHSecret(privateKey, publicKey) {
    ...
    async kdfRK(rootKey, dhOutput) {
        ...
        }, rootKey, 512 // 256 bits for root key + 256 bits for chain key
        );
        ...
        // Import as HKDF keys
        const newRootKey = await window.crypto.subtle.importKey('raw', newRootKeyBits, { name: 'HKDF' }, false, ['deriveBits', 'deriveKey']);
        const newChainKey = await window.crypto.subtle.importKey('raw', newChainKeyBits, { name: 'HKDF' }, false, ['deriveBits', 'deriveKey']);
        return {
            rootKey: newRootKey,
    ...
    async kdfCK(chainKey) {
        ...
        }, chainKey, 512 // 256 bits for message key + 256 bits for next chain key
        );
        // Split the derived bits
        const messageKeyBits = derivedBits.slice(0, 32);
        const nextChainKeyBits = derivedBits.slice(32, 64);
        // Import as keys
        const messageKey = await window.crypto.subtle.importKey('raw', messageKeyBits, { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']);
        const nextChainKey = await window.crypto.subtle.importKey('raw', nextChainKeyBits, { name: 'HKDF' }, false, ['deriveBits', 'deriveKey']);
        return {
            messageKey,
     ...
     */
    async deleteRatchet(recipientId) {
...
exports.KeyRatcheting = KeyRatcheting;
